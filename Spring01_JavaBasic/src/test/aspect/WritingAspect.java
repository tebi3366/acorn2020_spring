 ...
 * 	   point cut  aspect이 지정되는곳
 * 	2. execution(* test.service.*.*(..)) 
 * 		=> test.service 패키지의 모든 클래스 , 모든 메소드 point cut
 ...
 *      메소드가 point cut (aop 가 적용되는 위치)
 *      
 *     execution(* *(java.lang.String)) < 반드시 스트링타입을 포함하고잇어야함
 *     execution(* test.service.CafeService.add*(..)) 카페 서비스에 모든 add 메소드 
 *  
 *  6. execution(String update*(Integer,*))
 *  	=> 메소드명이 update로 시작하고 리턴 type 은 String
 *  	메소드의 첫번째 인자는 Integer type, 두번쨰 인자는 아무 type 다되는
 *  	메소드가 point cut(aop 가 적용되는 위치)
 *  
 */	 
